...
function vhost(opt) {
    var insensitive, redirect = redirect_body(301);
    function builder(moved, domain) {
        for (var i = 0, ii = moved.length; i < ii; ++i) moved[i] = expression(moved[i]);
        return {
            reg: moved,
            orig: domain
        };
    ...
    function redirect_body(code) {
        ...
        return function(res, moved, host, url) {
            for (var reg = moved.reg, i = 0, ii = reg.length; i < ii; ++i) if (reg[i].test(host)) {
    ...
    function proxies(domain, moved, proxy) {
        ...
        if (moved) {
            ...
            return function(req, res, next) {
                ...
                if (domainC.test(host)) proxyC.web(req, res); else if (!rdc(res, mvd, host, req.url)) return next();
            };
        }
        return function(req, res, next) {
            var host = req.headers.host;
    ...
    function dynamics(files) {
        ...
        return function(req, res, next) {
            for (var host = req.headers.host, data = JSON.parse(fs.readFileSync(file, "utf8")), i = 0, ii = data.length; i < ii; ++i) {
                var moved, d = data[i];
    ...
    var domain, fw, proxy, moved, temp, options = opt || Object.create(null), next = Object.create(null);
    ...
    if (options.file && console.info('top-vhost > "file" option is deprecated'), !0 === Array.isArray(options.redirect) && (moved = builder(options.redirect, options.domain.source || options.domain), 
    ...
    if (options.static) return function(files, obj) {
        ...
        if (obj.moved ? moved = obj.moved : !0 === Array.isArray(d.redirect) && (moved = builder(d.redirect, obj.orig || d.domain.source || d.domain)), 
        ...
        if (d.proxies && "object" == typeof d.proxies) return proxies(domain, moved, require("http-proxy").createProxyServer(d.proxies));
        throw new Error('"framework" or "proxies" are required');
    }(String(options.static), next);
